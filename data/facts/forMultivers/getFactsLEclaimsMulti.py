...
facts = None
with open("../factsOrigSents.txt") as fi:
    facts = fi.read()
facts = json.loads(facts)

# Get claims and write to file.
ind = 0
claims = {}
f = open("corpusLEclaims.jsonl", "a")
for t in facts.keys():
    claims[t] = []
    for fact in facts[t]:
        out = split_sents(fact)[0][0]
        for o in out:
            claims[t].append(o)
    f.write('{"doc_id": '+str(ind)+',"title": "'+str(t)+'","abstract": '+str(claims[t])+'}\n')
    ind += 1
f.close()

#NOTE: had to ctrl+F and replace one quote to two quotes in corpus.jsonl after generating it
